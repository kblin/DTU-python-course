...
- "Select multiple columns or rows using by location using `DataFrame.iloc`"
- "Select multiple columns or rows using by labels using `DataFrame.loc`"
...
## Use `DataFrame.loc[..., ...]` to select values by labels.

*   Can specify location by name/label.

~~~
data = pandas.read_csv('data/gapminder_gdp_europe.csv', index_col='country')
print(data.["Albania", "gdpPercap_1952"])
~~~
{: .python}
~~~
1601.056136
